...
int ili9881c_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	if (ret < 0) {
		if ((lcd_size_flag == 0) || (lcd_size_flag == 1) || (lcd_size_flag == 3)){
			printk("ili9881c_dsi_probe: use 7inch  display timing\n");
			videomode_from_timing(&asus_ili9881c_default_mode_7inch, &ctx->vm);
		} else if (lcd_size_flag==2) {
			printk("ili9881c_dsi_probe: use 10-inch  display timing\n");
			videomode_from_timing(&asus_ili9881c_default_mode_10inch, &ctx->vm);
		}
	}
