...
struct ov5645 {
	...
	void (*io_init)(struct ov5645 *ov5645_data);
...
static s32 ov5645_read_reg(struct ov5645 *ov5645_data, u16 reg, u8 *val);
static s32 ov5645_write_reg(struct ov5645 *ov5645_data, u16 reg, u8 val);
...
static inline void ov5645_power_down(struct ov5645 *ov5645_data, int enable)
{
	if (ov5645_data->pwn_gpio < 0)
		return;

	if (enable)
		gpio_set_value_cansleep(ov5645_data->pwn_gpio, 0);
	else
		gpio_set_value_cansleep(ov5645_data->pwn_gpio, 1);

	msleep(2);
}

static void ov5645_reset(struct ov5645 *ov5645_data)
{
	if (ov5645_data->rst_gpio < 0 || ov5645_data->pwn_gpio < 0)
		return;

	/* camera reset */
	gpio_set_value(ov5645_data->rst_gpio, 1);

	/* camera power dowmn */
	gpio_set_value(ov5645_data->pwn_gpio, 0);
	msleep(5);

	gpio_set_value(ov5645_data->pwn_gpio, 1);
	msleep(5);

	gpio_set_value(ov5645_data->rst_gpio, 0);
	msleep(1);
...
static int ov5645_regulator_enable(struct ov5645 *ov5645_data)
{
	int ret = 0;
	struct device *dev = &ov5645_data->i2c_client->dev;

	ov5645_data->io_regulator = devm_regulator_get(dev, "DOVDD");
	if (!IS_ERR(ov5645_data->io_regulator)) {
		regulator_set_voltage(ov5645_data->io_regulator,
				      OV5645_VOLTAGE_DIGITAL_IO,
				      OV5645_VOLTAGE_DIGITAL_IO);
		ret = regulator_enable(ov5645_data->io_regulator);
		if (ret) {
		...
		ov5645_data->io_regulator = NULL;
	}
	ov5645_data->core_regulator = devm_regulator_get(dev, "DVDD");
	if (!IS_ERR(ov5645_data->core_regulator)) {
		regulator_set_voltage(ov5645_data->core_regulator,
				      OV5645_VOLTAGE_DIGITAL_CORE,
				      OV5645_VOLTAGE_DIGITAL_CORE);
		ret = regulator_enable(ov5645_data->core_regulator);
		if (ret) {
		...
		ov5645_data->core_regulator = NULL;
		pr_err("%s: cannot get core voltage error\n", __func__);
	}
	ov5645_data->analog_regulator = devm_regulator_get(dev, "AVDD");
	if (!IS_ERR(ov5645_data->analog_regulator)) {
		regulator_set_voltage(ov5645_data->analog_regulator,
				      OV5645_VOLTAGE_ANALOG,
				      OV5645_VOLTAGE_ANALOG);
		ret = regulator_enable(ov5645_data->analog_regulator);
		if (ret) {
		...
		ov5645_data->analog_regulator = NULL;
...
static void ov5645_release_af(struct ov5645 *ov5645_data)
{
	ov5645_write_reg(ov5645_data, 0x3022, 0x08);
}

static void ov5645_enable_cont_af(struct ov5645 *ov5645_data)
{
	ov5645_release_af(ov5645_data);
	ov5645_write_reg(ov5645_data, 0x3022, 0x12);
	ov5645_write_reg(ov5645_data, 0x3022, 0x04);
}

static void OV5645_stream_on(struct ov5645 *ov5645_data)
{
	ov5645_write_reg(ov5645_data, 0x3008, 0x02);
	ov5645_write_reg(ov5645_data, 0x4202, 0x00);
	ov5645_enable_cont_af(ov5645_data);
}

static void OV5645_stream_off(struct ov5645 *ov5645_data)
{
	ov5645_release_af(ov5645_data);
	ov5645_write_reg(ov5645_data, 0x4202, 0x0f);
	ov5645_write_reg(ov5645_data, 0x3008, 0x42);
}

static int OV5645_get_sysclk(struct ov5645 *ov5645_data)
{
	 /* calculate sysclk */
	...
	temp1 = ov5645_read_reg(ov5645_data, 0x3034, &temp);
	temp2 = temp1 & 0x0f;
	if (temp2 == 8 || temp2 == 10)
		Bit_div2x = temp2 / 2;

	temp1 = ov5645_read_reg(ov5645_data, 0x3035, &temp);
	SysDiv = temp1>>4;
	if (SysDiv == 0)
		SysDiv = 16;

	temp1 = ov5645_read_reg(ov5645_data, 0x3036, &temp);
	Multiplier = temp1;

	temp1 = ov5645_read_reg(ov5645_data, 0x3037, &temp);
	PreDiv = temp1 & 0x0f;
	Pll_rdiv = ((temp1 >> 4) & 0x01) + 1;

	temp1 = ov5645_read_reg(ov5645_data, 0x3108, &temp);
	temp2 = temp1 & 0x03;
...
static void OV5645_set_night_mode(struct ov5645 *ov5645_data)
{
	 /* read HTS from register settings */
	u8 mode;

	ov5645_read_reg(ov5645_data, 0x3a00, &mode);
	mode &= 0xfb;
	ov5645_write_reg(ov5645_data, 0x3a00, mode);
}

static int OV5645_get_HTS(struct ov5645 *ov5645_data)
{
	 /* read HTS from register settings */
	int HTS;
	u8 temp;

	HTS = ov5645_read_reg(ov5645_data ,0x380c, &temp);
	HTS = (HTS<<8) + ov5645_read_reg(ov5645_data, 0x380d, &temp);

	return HTS;
}

static int OV5645_get_VTS(struct ov5645 *ov5645_data)
{
	...
	VTS = ov5645_read_reg(ov5645_data, 0x380e, &temp);

	VTS = (VTS<<8) + ov5645_read_reg(ov5645_data, 0x380f, &temp);

	return VTS;
}

static int OV5645_set_VTS(struct ov5645 *ov5645_data, int VTS)
{
	 /* write VTS to registers */
	 int temp;

	 temp = VTS & 0xff;
	 ov5645_write_reg(ov5645_data, 0x380f, temp);

	 temp = VTS>>8;
	 ov5645_write_reg(ov5645_data, 0x380e, temp);

	 return 0;
}

static int OV5645_get_shutter(struct ov5645 *ov5645_data)
{
	 /* read shutter, in number of line period */
	int shutter;
	u8 temp;

	shutter = (ov5645_read_reg(ov5645_data, 0x03500, &temp) & 0x0f);
	shutter = (shutter<<8) + ov5645_read_reg(ov5645_data, 0x3501, &temp);
	shutter = (shutter<<4) + (ov5645_read_reg(ov5645_data, 0x3502, &temp)>>4);

	 return shutter;
}

static int OV5645_set_shutter(struct ov5645 *ov5645_data, int shutter)
{
	 ...
	 temp = shutter & 0x0f;
	 temp = temp<<4;
	 ov5645_write_reg(ov5645_data, 0x3502, temp);

	 temp = shutter & 0xfff;
	 temp = temp>>4;
	 ov5645_write_reg(ov5645_data, 0x3501, temp);

	 temp = shutter>>12;
	 ov5645_write_reg(ov5645_data, 0x3500, temp);

	 return 0;
}

static int OV5645_get_gain16(struct ov5645 *ov5645_data)
{
	 /* read gain, 16 = 1x */
	int gain16;
	u8 temp;

	gain16 = ov5645_read_reg(ov5645_data, 0x350a, &temp) & 0x03;
	gain16 = (gain16<<8) + ov5645_read_reg(ov5645_data, 0x350b, &temp);

	return gain16;
}

static int OV5645_set_gain16(struct ov5645 *ov5645_data, int gain16)
{
	/* write gain, 16 = 1x */
	u8 temp;
	gain16 = gain16 & 0x3ff;
	...
	temp = gain16 & 0xff;
	ov5645_write_reg(ov5645_data, 0x350b, temp);

	temp = gain16>>8;
	ov5645_write_reg(ov5645_data, 0x350a, temp);

	return 0;
}

static int OV5645_get_light_freq(struct ov5645 *ov5645_data)
{
	/* get banding filter value */
	int temp, temp1, light_freq = 0;
	u8 tmp;

	temp = ov5645_read_reg(ov5645_data, 0x3c01, &tmp);

	if (temp & 0x80) {
		/* manual */
		temp1 = ov5645_read_reg(ov5645_data, 0x3c00, &tmp);
		if (temp1 & 0x04) {
			/* 50Hz */
			light_freq = 50;
		} else {
		...
		/* auto */
		temp1 = ov5645_read_reg(ov5645_data, 0x3c0c, &tmp);
		if (temp1 & 0x01) {
			/* 50Hz */
			light_freq = 50;
		} else {
...
static void OV5645_set_bandingfilter(struct ov5645 *ov5645_data)
{
	int prev_VTS;
	int band_step60, max_band60, band_step50, max_band50;

	/* read preview PCLK */
	ov5645_data->prev_sysclk = OV5645_get_sysclk(ov5645_data);
	/* read preview HTS */
	ov5645_data->prev_HTS = OV5645_get_HTS(ov5645_data);

	/* read preview VTS */
	prev_VTS = OV5645_get_VTS(ov5645_data);

	/* calculate banding filter */
	/* 60Hz */
	band_step60 = ov5645_data->prev_sysclk * 100/ov5645_data->prev_HTS * 100/120;
	ov5645_write_reg(ov5645_data, 0x3a0a, (band_step60 >> 8));
	ov5645_write_reg(ov5645_data, 0x3a0b, (band_step60 & 0xff));

	max_band60 = (int)((prev_VTS-4)/band_step60);
	ov5645_write_reg(ov5645_data, 0x3a0d, max_band60);

	/* 50Hz */
	band_step50 = ov5645_data->prev_sysclk * 100/ov5645_data->prev_HTS;
	ov5645_write_reg(ov5645_data, 0x3a08, (band_step50 >> 8));
	ov5645_write_reg(ov5645_data, 0x3a09, (band_step50 & 0xff));

	max_band50 = (int)((prev_VTS-4)/band_step50);
	ov5645_write_reg(ov5645_data, 0x3a0e, max_band50);
}

static int OV5645_set_AE_target(struct ov5645 *ov5645_data, int target)
{
	/* stable in high */
	int fast_high, fast_low;
	ov5645_data->AE_low = target * 23 / 25;	/* 0.92 */
	ov5645_data->AE_high = target * 27 / 25;	/* 1.08 */

	fast_high = ov5645_data->AE_high<<1;
	if (fast_high > 255)
		fast_high = 255;

	fast_low = ov5645_data->AE_low >> 1;

	ov5645_write_reg(ov5645_data, 0x3a0f, ov5645_data->AE_high);
	ov5645_write_reg(ov5645_data, 0x3a10, ov5645_data->AE_low);
	ov5645_write_reg(ov5645_data, 0x3a1b, ov5645_data->AE_high);
	ov5645_write_reg(ov5645_data, 0x3a1e, ov5645_data->AE_low);
	ov5645_write_reg(ov5645_data, 0x3a11, fast_high);
	ov5645_write_reg(ov5645_data, 0x3a1f, fast_low);

	return 0;
}

static void OV5645_turn_on_AE_AG(struct ov5645 *ov5645_data, int enable)
{
	u8 ae_ag_ctrl;

	ov5645_read_reg(ov5645_data, 0x3503, &ae_ag_ctrl);
	if (enable) {
		/* turn on auto AE/AG */
	...
	ov5645_write_reg(ov5645_data, 0x3503, ae_ag_ctrl);
}

static bool binning_on(struct ov5645 *ov5645_data)
{
	u8 temp;
	ov5645_read_reg(ov5645_data, 0x3821, &temp);
	temp &= 0xfe;
	if (temp)
		return true;
	else
...
static void ov5645_set_virtual_channel(struct ov5645 *ov5645_data, int channel)
{
	u8 channel_id;

	ov5645_read_reg(ov5645_data, 0x4814, &channel_id);
	channel_id &= ~(3 << 6);
	ov5645_write_reg(ov5645_data, 0x4814, channel_id | (channel << 6));
}

/* download ov5645 settings to sensor through i2c */
static int ov5645_download_firmware(struct ov5645 *ov5645_data, struct reg_value *pModeSetting, s32 ArySize)
{
	...
	for (i = 0; i < ArySize; ++i, ++pModeSetting) {
		...
		if (Mask) {
			retval = ov5645_read_reg(ov5645_data, RegAddr, &RegVal);
			if (retval < 0)
				goto err;
		...
		retval = ov5645_write_reg(ov5645_data, RegAddr, Val);
		if (retval < 0)
			goto err;
...
static void ov5645_dnld_af_fw(struct ov5645 *ov5645_data)
{
	...
	retval = ov5645_download_firmware(ov5645_data, pModeSetting, ArySize);

	do {
		ov5645_read_reg(ov5645_data, 0x3029, &temp);
		msleep(5);
 ...
 */
static int ov5645_change_mode_exposure_calc(struct ov5645 *ov5645_data,
				enum ov5645_frame_rate frame_rate, enum ov5645_mode mode)
{
	struct reg_value *pModeSetting = NULL;
	s32 ArySize = 0;
	u8 average;
	...
	/* turn off AE/AG */
	OV5645_turn_on_AE_AG(ov5645_data, 0);

	/* read preview shutter */
	prev_shutter = OV5645_get_shutter(ov5645_data);
	if ((binning_on(ov5645_data)) && (mode != ov5645_mode_720P_1280_720)
			&& (mode != ov5645_mode_1080P_1920_1080))
		prev_shutter *= 2;

	/* read preview gain */
	prev_gain16 = OV5645_get_gain16(ov5645_data);

	/* get average */
	ov5645_read_reg(ov5645_data, 0x56a1, &average);

	/* turn off night mode for capture */
	OV5645_set_night_mode(ov5645_data);

	/* turn off overlay */
	/* ov5645_write_reg(ov5645_data, 0x3022, 0x06);//if no af function, just skip it */

	OV5645_stream_off(ov5645_data);
	...
	/* read capture VTS */
	cap_VTS = OV5645_get_VTS(ov5645_data);
	cap_HTS = OV5645_get_HTS(ov5645_data);
	cap_sysclk = OV5645_get_sysclk(ov5645_data);

	/* calculate capture banding filter */
	light_freq = OV5645_get_light_freq(ov5645_data);
	if (light_freq == 60) {
		/* 60Hz */
	...
	if (average > ov5645_data->AE_low && average < ov5645_data->AE_high) {
		/* in stable range */
		cap_gain16_shutter =
		  prev_gain16 * prev_shutter * cap_sysclk/ov5645_data->prev_sysclk
		  * ov5645_data->prev_HTS/cap_HTS * ov5645_data->AE_Target / average;
	} else {
		cap_gain16_shutter =
		  prev_gain16 * prev_shutter * cap_sysclk/ov5645_data->prev_sysclk
		  * ov5645_data->prev_HTS/cap_HTS;
	}
	...
	/* write capture gain */
	OV5645_set_gain16(ov5645_data, cap_gain16);

	/* write capture shutter */
	if (cap_shutter > (cap_VTS - 4)) {
		cap_VTS = cap_shutter + 4;
		OV5645_set_VTS(ov5645_data, cap_VTS);
	}
	OV5645_set_shutter(ov5645_data, cap_shutter);
 ...
 * */
static int ov5645_change_mode_direct(struct ov5645 *ov5645_data,
				enum ov5645_frame_rate frame_rate, enum ov5645_mode mode)
{
	struct reg_value *pModeSetting = NULL;
	s32 ArySize = 0;
	...
	OV5645_turn_on_AE_AG(ov5645_data, 1);

err:
	return retval;
}

static int ov5645_init_mode(struct ov5645 *ov5645_data, enum ov5645_frame_rate frame_rate,
			    enum ov5645_mode mode, enum ov5645_mode orig_mode)
{
	...
	if (mode == ov5645_mode_INIT) {
		...
		ov5645_dnld_af_fw(ov5645_data);

	} else if ((dn_mode == SUBSAMPLING && orig_dn_mode == SCALING) ||
			(dn_mode == SCALING && orig_dn_mode == SUBSAMPLING)) {
		/* change between subsampling and scaling
		 * go through exposure calucation */
		retval = ov5645_change_mode_exposure_calc(ov5645_data, frame_rate, mode);
	} else {
		/* change inside subsampling or scaling
		 * download firmware directly */
		retval = ov5645_change_mode_direct(ov5645_data, frame_rate, mode);
	}

	if (retval < 0)
		goto err;

	OV5645_set_AE_target(ov5645_data, ov5645_data->AE_Target);
	OV5645_get_light_freq(ov5645_data);
	OV5645_set_bandingfilter(ov5645_data);
	ov5645_set_virtual_channel(ov5645_data, ov5645_data->csi);
...
static int ov5645_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		if ((u32)a->parm.capture.capturemode == 0) {
			a->parm.capture.capturemode = get_capturemode(sensor->pix.width,
				sensor->pix.height);
		}
 ...
 *
 */
static int init_device(struct ov5645 *ov5645_data)
{
	...
	ret = ov5645_init_mode(ov5645_data, frame_rate, ov5645_mode_INIT, ov5645_mode_INIT);
			...
			const struct i2c_device_id *id)
{
	...
	struct ov5645 *ov5645_data;
	int retval;
	u8 chip_id_high, chip_id_low;

	/* Set initial values for the sensor struct. */
	ov5645_data = devm_kzalloc(dev, sizeof(struct ov5645), GFP_KERNEL);
	if (!ov5645_data)
		return -ENOMEM;
	...
	ov5645_data->pwn_gpio = of_get_named_gpio(dev->of_node, "pwn-gpios", 0);
	if (!gpio_is_valid(ov5645_data->pwn_gpio))
		dev_warn(dev, "no sensor pwdn pin available");
	else {
		retval = devm_gpio_request_one(dev, ov5645_data->pwn_gpio, GPIOF_OUT_INIT_HIGH,
						"ov5645_mipi_pwdn");
		if (retval < 0) {
	...
	/* request reset pin */
	ov5645_data->rst_gpio = of_get_named_gpio(dev->of_node, "rst-gpios", 0);
	if (!gpio_is_valid(ov5645_data->rst_gpio))
		dev_warn(dev, "no sensor reset pin available");
	else {
		retval = devm_gpio_request_one(dev, ov5645_data->rst_gpio, GPIOF_OUT_INIT_HIGH,
						"ov5645_mipi_reset");
		if (retval < 0) {
	...
	retval = of_property_read_u32(dev->of_node, "ae_target", &(ov5645_data->AE_Target));
	if (retval) {
		dev_warn(dev,
			 "ae_target missing in dev tree, using default %d\n",
			 ov5645_data->AE_Target);
	}
	...
	ov5645_regulator_enable(ov5645_data);

	ov5645_reset(ov5645_data);

	ov5645_power_down(ov5645_data, 0);

	retval = ov5645_read_reg(ov5645_data, OV5645_CHIP_ID_HIGH_BYTE, &chip_id_high);
	if (retval < 0 || chip_id_high != 0x56) {
		pr_warning("camera ov5645_mipi is not found\n");
		clk_disable_unprepare(ov5645_data->sensor_clk);
		return -ENODEV;
	}
	retval = ov5645_read_reg(ov5645_data, OV5645_CHIP_ID_LOW_BYTE, &chip_id_low);
	if (retval < 0 || chip_id_low != 0x45) {
		pr_warning("camera ov5645_mipi is not found\n");
		clk_disable_unprepare(ov5645_data->sensor_clk);
		return -ENODEV;
	}
	retval = init_device(ov5645_data);
	if (retval < 0) {
		clk_disable_unprepare(ov5645_data->sensor_clk);
		pr_warning("camera ov5645 init failed\n");
		ov5645_power_down(ov5645_data, 1);
		return retval;
	}
...
static int ov5645_remove(struct i2c_client *client)
{
	...
	struct ov5645 *ov5645_data = to_ov5645(client);
	...
	if (ov5645_data->gpo_regulator)
		regulator_disable(ov5645_data->gpo_regulator);

	if (ov5645_data->analog_regulator)
		regulator_disable(ov5645_data->analog_regulator);

	if (ov5645_data->core_regulator)
		regulator_disable(ov5645_data->core_regulator);

	if (ov5645_data->io_regulator)
		regulator_disable(ov5645_data->io_regulator);
